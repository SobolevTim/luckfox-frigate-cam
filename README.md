...
Параметры изображения и стрима управляются только через **MQTT** с автоматическим  
HA Discovery — все сущности появляются в Home Assistant автоматически без ручной  
конфигурации. HTTP API в проекте больше не используется.
...
| | Оригинал (`luckfox_pico_rtsp_opencv`) | Этот проект |
|---|---|---|
| OpenCV | ✅ Используется для YUV→RGB | ❌ Не нужен |
| Конвертация цвета | CPU (медленно) | Нет — NV12 напрямую |
| FPS оверлей | Есть | Нет (нужна чистая картинка для Frigate) |
| ISP контроль | Нет | Полный (яркость, WB, день/ночь…) |
| Управление стримом | Базовое | Битрейт + FPS на лету |
| Аудио RTSP | Нет | 🧪 Экспериментально (`G.711A`, только Pico Ultra) |
| HTTP API | Нет | Нет |
| MQTT + HA Discovery | Нет | Есть |
| Clamp / валидация входных значений | Нет | Есть |
| Сохранение настроек | Нет | `/etc/camera_rtsp.json` |
| Чистое завершение | Частичное | Полное (Ctrl+C / SIGTERM) |
