...
### v0.2.1
- **Исправлено:** NR-профиль ночного режима (настройки шумоподавления) не применялся после перезагрузки, если `night_mode=ON` был сохранён в `/etc/camera_rtsp.json`. `isp_apply_settings` теперь вызывает `apply_night_hw_profile()` напрямую, обходя защиту от повторного применения в `isp_set_night_mode`.
- **Исправлено:** При старте с сохранённым `night_mode=ON` кэш дневного профиля (`g_day_hw`) не заполнялся, из-за чего переключение ночь→день сбрасывало fps и bitrate в захардкоженные константы. Теперь кэш инициализируется значениями по умолчанию при старте.
- **Исправлено:** `vi_chn_init` объединял возвращаемые значения `SetChnAttr` и `EnableChn` через OR — `EnableChn` вызывался даже при неудаче `SetChnAttr`. Теперь каждый вызов проверяется отдельно.
- **Исправлено:** LWT Will-сообщение переведено с QoS-0 на QoS-1, что гарантирует доставку `availability=offline` брокеру даже при потере пакетов.
- **Исправлено:** Буфер выходных пакетов VENC увеличен с 1 до `MAX_VENC_PACKS=4`; цикл передачи теперь итерирует по всем возвращённым пакетам, устраняя переполнение кучи на мультипакетных кадрах (SPS+PPS+IDR).
- **Исправлено:** `isp_get_settings` больше не изменяет `g_cfg` через `sanitize_settings`; санитизация применяется только к возвращаемой копии.
- **Исправлено:** `RkLunch-stop.sh` теперь вызывается по абсолютному пути `/usr/bin/RkLunch-stop.sh` для надёжной работы в systemd-окружениях с урезанным `$PATH`.
- **Исправлено:** При ошибке `vi_chn_init` VI-устройство теперь корректно деактивируется (`RK_MPI_VI_DisableDev`), освобождая сенсор для повторной попытки запуска.
- **Исправлено:** Детектирование birth-сообщения HA изменено с `pay_len >= 6` на `pay_len == 6`, чтобы только точное значение `"online"` триггерило переотправку discovery.
